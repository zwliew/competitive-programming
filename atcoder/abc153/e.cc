...
int minpts(vector<pair<int, int>> &spells, vector<vector<int>> &mem, int idx,
           int curhealth) {
  if (curhealth <= 0) {
    return 0;
  }
  ...
  if (mem[idx][curhealth] != -1) {
    return mem[idx][curhealth];
  }

  mem[idx][curhealth] =
      min(minpts(spells, mem, idx, curhealth - spells[idx].first) +
              spells[idx].second,
          minpts(spells, mem, idx - 1, curhealth));
  return mem[idx][curhealth];
}
...
int main() {
  ...
  vector<vector<int>> mem(n + 1, vector<int>(h + 1, -1));
  for (int i = 0; i < n; ++i) {
    int a, b;
    cin >> a >> b;
    spells[i] = mp(a, b);
