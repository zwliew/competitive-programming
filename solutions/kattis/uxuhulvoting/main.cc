...
int dp(vector<vector<int>> &mem, vector<vector<int>> &p, int idx, int cur) {
  if (idx == p.size()) {
    return cur;
  }
  ...
  for (int i = 0; i < 3; ++i) {
    int next = dp(mem, p, idx + 1, cur ^ (1 << i));
    if (mem[idx][cur] == -1 || p[idx][next] < p[idx][mem[idx][cur]]) {
      mem[idx][cur] = next;
    }
  }
  return mem[idx][cur];
...
int main() {
  ...
  vector<string> mapping = {"NNN", "NNY", "NYN", "NYY",
                            "YNN", "YNY", "YYN", "YYY"};
  ...
  while (n--) {
    ...
    vector<vector<int>> mem(m + 1, vector<int>(9, -1));
