...
VI ds;
s32 num;
inline void initSet(s32 n) {
  ds = VI(n);
  iota(ds.begin(), ds.end(), 0);
  num = n;
}
inline s32 findSet(s32 a) { return ds[a] == a ? a : (ds[a] = findSet(ds[a])); }
inline bool isSameSet(s32 a, s32 b) { return findSet(a) == findSet(b); }
inline void unionSet(s32 a, s32 b) {
  if (isSameSet(a, b)) return;
  ds[findSet(a)] = findSet(b);
  --num;
}
