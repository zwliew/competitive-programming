...
int main() {
  ...
  fin >> a >> b;
  s32 an = 1, bn = 1;
  REP(i, a.size()) { an *= a[i] - 'A' + 1; }
  REP(i, b.size()) { bn *= b[i] - 'A' + 1; }

  fout << ((an % 47) == (bn % 47) ? "GO" : "STAY") << '\n';
} /*
 ID: zhaowei10
 TASK:
 LANG: C++
...
int main() {
  ...
  while (e--) {
    s32 a, b, d;
    cin >> a >> b >> d;
    ev[a].emplace_back(d, b);
    ev[b].emplace_back(d, a);
  }
  ...
  while (!q.empty()) {
    ...
    for (II n : ev[x.second]) {
      q.emplace(max(n.first, x.first), n.second);
    }
  }
